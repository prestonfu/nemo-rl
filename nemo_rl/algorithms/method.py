...
class MethodSaveState(TypedDict):
...
def _default_method_save_state() -> MethodSaveState:
    return {
        "current_epoch": 0,
        "current_step": 0,
        "total_steps": 0,
...
def setup(
    ...
    """Main entry point for method algorithm.

    Returns:
        tuple of student_policy, teacher_policy, student_generation,
        train_dataloader, val_dataloader,
        loss_fn, logger, checkpointer, method_save_state, master_config
    """
    ...
    assert generation_config is not None, (
        "A generation config in the PolicyConfig is required for method"
    )
    ...
    for cfg, who in ((policy_config, "student"), (teacher_config, "teacher")):
        ...
        # Therefore, we disable SP + packing for this method.
        ...
        if dtensor_enabled and sequence_packing_enabled and sequence_parallel_enabled:
            raise AssertionError(
                f"Method does not support DTensor sequence parallel + sequence packing ({who} policy). "
                "Please refer to https://github.com/NVIDIA-NeMo/RL/issues/1178 for more details."
            )

    # Set random seed
    set_seed(method_config["seed"])
    ...
    method_save_state: Optional[MethodSaveState] = cast(
        Optional[MethodSaveState],
        checkpointer.load_training_info(last_checkpoint_path),
    )
    if method_save_state is None:
        method_save_state = _default_method_save_state()
    ...
    dataloader = StatefulDataLoader(
        train_dataset,
        batch_size=method_config["num_prompts_per_step"],
    ...
    if (
        method_config["val_period"] > 0
        or method_config["val_at_start"]
        or method_config["val_at_end"]
    ):
        ...
        val_dataloader = StatefulDataLoader(
            val_dataset,
            batch_size=method_config["val_batch_size"],
            shuffle=False,
            collate_fn=rl_collate_fn,
        )
        print(
    ...
    if "megatron_cfg" in teacher_config and teacher_config["megatron_cfg"]["enabled"]:
        ...
        total_train_iters = min(
            method_config["max_num_steps"],
            method_config["max_num_epochs"] * len(dataloader),
        )
...
def method_train(
    ...
    method_save_state: MethodSaveState,
    master_config: MasterConfig,
) -> None:
    """Run method training algorithm."""
    timer = Timer()
    ...
    current_epoch = method_save_state["current_epoch"]  # current epoch
    current_step = method_save_state[
        "current_step"
    ]  # current step within current epoch
    total_steps = method_save_state[
        "total_steps"
    ]  # total number of steps across all epochs
    consumed_samples = method_save_state["consumed_samples"]
    total_valid_tokens = method_save_state["total_valid_tokens"]
    val_period = master_config["method"]["val_period"]
    val_at_start = master_config["method"]["val_at_start"]
    val_at_end = master_config["method"]["val_at_end"]
    colocated_inference = master_config["policy"]["generation"]["colocated"]["enabled"]
    max_epochs = master_config["method"][
        "max_num_epochs"
    ]  # max number of epochs to train for
    max_steps = master_config["method"][
        "max_num_steps"
    ...
    # Run method training (multi-epoch until reaching max_num_steps or max_num_epochs)
            ...
            with timer.time("total_step_time"):
                ...
                with timer.time("data_processing"):
                    ...
                    repeated_batch: BatchedDataDict[DatumSpec] = (
                        batch.repeat_interleave(
                            master_config["method"]["num_generations_per_prompt"]
                        )
                    )
                ...
                with timer.time("generation"):
                    ...
                    if _should_use_async_rollouts(master_config):
                        (
                            ...
                            max_rollout_turns=master_config["method"][
                                "max_rollout_turns"
                            ],
                            greedy=False,
                        )
                ...
                with timer.time("teacher_logprob_inference"):
                    teacher_topk = teacher_policy.get_topk_logits(
                        train_data,
                        k=master_config["method"]["topk_logits_k"],
                        timer=timer,
                    )
                ...
                ## Checkpointing
                consumed_samples += master_config["method"][
                ...
                if master_config["checkpointing"]["enabled"] and (
                    ...
                    method_save_state["current_epoch"] = current_epoch
                    method_save_state["current_step"] = current_step + 1
                    method_save_state["total_steps"] = total_steps + 1
                    method_save_state["total_valid_tokens"] = total_valid_tokens
                    if val_metrics is not None:
                        method_save_state["val_reward"] = val_metrics["accuracy"]
                    elif "val_reward" in method_save_state:
                        del method_save_state["val_reward"]
                    method_save_state["consumed_samples"] = consumed_samples
                    ...
                    if full_metric_name is not None:
                        ...
                        if not metrics_source:
                            ...
                            if full_metric_name in method_save_state:
                                del method_save_state[full_metric_name]
                        elif metric_name not in metrics_source:
                            raise ValueError(
                                f"Metric {metric_name} not found in {prefix} metrics"
                            )
                        else:
                            method_save_state[full_metric_name] = metrics_source[
                                metric_name
                            ]
                    ...
                    with timer.time("checkpointing"):
                        ...
                        checkpoint_path = checkpointer.init_tmp_checkpoint(
                            total_steps + 1, method_save_state, master_config
                        )
...
def validate(
    ...
    with timer.time("total_validation_time"):
        ...
        max_batches = (
            master_config["method"]["max_val_samples"]
            // master_config["method"]["val_batch_size"]
        )
